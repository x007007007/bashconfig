...
function bash_config_change_checker () {
	local filepath filename template_filepath output_file
	filepath=$1
	if [[ ! -f ${filepath} ]]; then
  ...
  template_filepath=${bash_template_path}/${filename:1}
  if [[ ! -f ${template_filepath} ]]; then
		echo "ERROR Template ${template_filepath} not exist" 1>&2
		return
	fi
  output_file=${bash_intelligent_root}/changelog/${filename:1}-$(date +%y%m%d_%H%M%S).bash
